...
namespace ObjectRepresentation
{
    public class SetUpForObjects : MonoBehaviour
    {
        ...
        private void CreateTimelineObjects(GameObject spawnObject,GameObject gazeSpawnObject,GameObject timelineObjects, GameVisibilityControl timelineVisibilityControl, Color color, Game game, List<List<Vector3>> gameWorldPositions, List<Vector3> gazeWorldPositions)
        {
            ...
            spawner.color = color;
            spawner.gazeObject = gazeSpawnObject;
            spawner.instanceObject = spawnObject;
        ...
        private void ThreeDSetUp()
        {
            ...
            for (var j = 0; j < storage.GameList.Count; j++)
            {
                ...
                CreateTimelineObjects(instanceObject,gazeObject,timelineObjects,timelineVisibilityControl, color, game, objectWorldPositions, gazeWorldPositions);
                CreateTimeframeObjects(timeframeObjects,game,objectWorldPositions,timeframeVisibilityControl,color,gazeWorldPositions, j);
                i++;
            }
        }

        private void TwoDSetUp()
        {
            var timelineObjects = new GameObject
            {
                name = "TimelineObjects"
            ...
            for (var j = 0; j < storage.GameList.Count; j++)
            {
                ...
                foreach (var obj in game.Objects)
                {
                    ...
                    foreach (var point in obj.Points)
                    {
                        var pos = new Vector3(point.PosX*ratioWidth, point.PosY*ratioHeight, point.PosZ);
                        var worldPos = storage.MainCamera.ScreenToWorldPoint(pos);
                        pointsInWorldCoords.Add(worldPos);
                    }
                    objectWorldPositions.Add(pointsInWorldCoords);
                ...
                foreach (var gazePoint in storage.SensorData[i])
                {
                    var pos = new Vector3(gazePoint.PosX*ratioWidth,gazePoint.PosY*ratioHeight,10);
                    var worldPos = storage.MainCamera.ScreenToWorldPoint(pos);
                    gazeWorldPositions.Add(worldPos);
                }
                
                CreateTimelineObjects(imageObject,imageGazeObject,timelineObjects,timelineVisibilityControl, color, game, objectWorldPositions, gazeWorldPositions);
                CreateTimeframeObjects(timeframeObjects,game,objectWorldPositions,timeframeVisibilityControl,color,gazeWorldPositions, j);
                i++;
            }
        }
